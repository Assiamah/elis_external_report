...
var selectControlHover = new OpenLayers.Control.SelectFeature(districtmap, {
	...
	overFeature: function(feature) {
		var districtNameLength = 150; //feature.attributes.districtname.length.valueOf();
		...
		districtmap.drawFeature(districtmap.getFeatureById(feature.id), {fillColor: "#FFCC00", fillOpacity: 0.1, strokeColor: "#00ffff"});
          if (feature.attributes.districtname.length >= 25){var w = 300}else{ if (feature.attributes.districtname.length >= 19){var w = 200}else{var w = 150}};
        var popupDistrict = new OpenLayers.Popup(
          "chicken",
          feature.geometry.getBounds().getCenterLonLat(),
          new OpenLayers.Size(w,80),
          "<div><strong>"+feature.attributes.districtname+
         "<br>Area: </strong>"+number_format(((feature.geometry.getGeodesicArea(proj900913))/1000).toFixed(2), 0, '.', ',')+" km<sup><small>2</small></sup>"+"</div>"+
         "<br><strong>Parcels: </strong>"+number_format(feature.attributes.numupn, 0, '.', ',')+"</div>",
          null,
          true,
          null);

       feature.popup = popupDistrict;

       mapLogin.addPopup(popupDistrict);
...
var selectControl = new OpenLayers.Control.SelectFeature( regionmap, {
    hover: true,
...
//-----------------------------------------------------------------------------
		...
		//it takes the request feed from getdristic.php from layer districts
//-----------------------------------------------------------------------------
