$(document).ready(function() {
  ...
  $.ajax({
      ...
      success: function(response) {
          ...
          // var approved_percentage = (json_result.approved_count/ json_result.count * 100);

          // let approved_percentage = isNaN(((json_result.approved_count / json_result.count) * 100).toFixed(2)) ? 0 : ((json_result.approved_count / json_result.count) * 100).toFixed(2);

          
          

          // console.log(approved_percentage);
  ...
  $('#view_applications_with_monitoring_unit_table').DataTable().clear().destroy();
    ...
    for(let i=0; i<json_result.apps_at_division.length; i++) {
      ...
      // console.log(business_process_name);
...
let action = `<div class="btn-group" role="group">
  ...
  <div class="dropdown-menu">
      <button class="dropdown-item" 
          ...
          id="btnAddToBatchlist-${json_result.apps_at_division[i].job_number}" 
          data-job_number="${json_result.apps_at_division[i].job_number}" 
          data-ar_name="${json_result.apps_at_division[i].ar_name}" 
          data-business_process_name="${json_result.apps_at_division[i].business_process_name}" 
          data-application_stage="${json_result.apps_at_division[i].application_stage}" 
          data-application_stage_name="${json_result.apps_at_division[i].application_stage_name}" 
          data-application_stage_baby_step="${json_result.apps_at_division[i].application_stage_baby_step}" 
          data-application_stage_name_baby_step="${json_result.apps_at_division[i].application_stage_name_baby_step}" 
          data-job_purpose="${json_result.apps_at_division[i].job_purpose}" 
          data-business_process_sub_name="${json_result.apps_at_division[i].business_process_sub_name}" 
          data-target="#askForPurposeOfSendingRequesttoOfficer" 
          data-toggle="modal">Add To Batchlist <i class="fas fa-list"></i>
      </button>
...
//       $('#bl_business_process_sub_name').val($(e.relatedTarget).data('business_process_name'));
...
//       console.log($(e.relatedTarget).data('business_process_name'));
...
$("#allmessageform").on("submit", function (event) {
  event.preventDefault();
  ...
  data = {
    "request_type": $("#allsendMessageModal").find("#request_type").val(),
    "officer_id": $("#allsendMessageModal").find("#officer_id").val(),
    "officer_name": $("#allsendMessageModal").find("#officer_name").val(),
    "job_numbers" : $("#allsendMessageModal").find("#job_numbers").val(),
  "message_type":$("#allsendMessageModal").find("#message_type").val(),
  "message": $("#allsendMessageModal").find("#message").val()
  }
  ...
  $('#apps_with_officers').on('click', function(e) {
   ...
   $('#view_applications_incoming_with_monitoring_unit_table').DataTable().clear().destroy();
     ...
     for(let i=0; i<json_result.apps_at_division.length; i++) {
       ...
       let job_status = json_result.apps_at_division[i].job_status;
      //  let job_purpose = json_result.apps_at_division[i].job_purpose;
       let job_recieved_by = json_result.apps_at_division[i].job_recieved_by;
