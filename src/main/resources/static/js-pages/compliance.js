...
//   });
  ...
  function refreshDashboardStats() {
    if ($("#page_name").text() === "compliance" || $("#page_name").text() === "compliance_no_login") {
      const region_id = $('#sel_change_region_compliance').val();
      
      submitAjax("ComplianceReport", "report_dashboard_all", { region_id: region_id }, function (data) {
        ...
        showDivisionSummaryUpdatedQ("#app-received-year", data.apps_rec_divisional, 'info');
        showDivisionSummaryUpdatedQ("#app-completed-year", data.apps_comp_divisional, 'success');
        showDivisionSummaryUpdatedQ("#app-received-completed-year", data.apps_comp_divisional_year, 'default');
...
window.toggleMonitorMode = function() {
    if (!isMonitorMode) {
        ...
        // Disable interactive clicks in monitor mode
        $(document).off('click', '.loadWarningApps');
        $(document).off('click', '.loadSuccessApps');
        $(document).off('click', '.loadInfoApps');
        $(document).off('click', '.loadDefaultApps');
        $(document).off('click', '.loadDangerApps');
        ...
        if (document.fullscreenElement) {
            ...
            // If already not in fullscreen, just stop the monitoring
            exitMonitorMode();
        }
    }
};
...
function loadMonitorDashboard() {
    ...
    const region_id = $('#sel_change_region_compliance').val();
    const region_name = $('#sel_change_region_compliance option:selected').text();
    
    // Update region label in header
    if (region_name && region_name !== 'Select Region') {
        $('#mon-current-region').text(region_name).show();
    } else {
        $('#mon-current-region').text('All Regions').show();
    }
    ...
    $.ajax({
        ...
        data: {
            request_type: 'report_dashboard_all',
            region_id: region_id,
            start_date: startDate,
            end_date: endDate
        },
        cache: false,
        timeout: 120000, // ✅ 2 minutes — give it enough time
        ...
        error: function(xhr, status, error) {
            ...
            if (status === 'timeout') {
                // ✅ On timeout — if we have cached data, use it
                if (dashboardCache) {
                    console.warn('Timeout — using cached data');
                    renderMonitorDashboard(dashboardCache);
                    $('#monitorLastUpdate').html(`
                        <span class="text-warning">
                            <i class="ri-time-line me-1"></i>
                            Cached (${new Date(cacheTimestamp).toLocaleTimeString('en-GB', {
                                hour: '2-digit', minute: '2-digit'
                            })})
                        </span>
                    `);
                } else {
                    showMonitorError('Request timed out. Retrying...');
                    scheduleRetry();
                }
            } else {
                showMonitorError(`Error: ${status}. Retrying in 2 minutes...`);
                scheduleRetry();
            }
        }
    });
...
function flashUpdateIndicator() {
    ...
    $('#monitorLastUpdate').html(`
        <span class="text-success">
            <i class="ri-checkbox-circle-line me-1"></i>${now}
        </span>
    `);

    // Flash the border of stat cards to show refresh happened
    $('.stat-card-monitor').css('border-color', 'rgba(16,185,129,0.8)');
    setTimeout(() => {
        $('.stat-card-monitor').css('border-color', 'rgba(42, 92, 143, 0.3)');
    }, 1500);
}
...
function renderMonitorDashboard(data) {
    ...
    const recMonth  = data.apps_rec_month?.[0]?.total || 0;
    ...
    animateCounter('mon-rec-month',      recMonth);
    $('#mon-completion-rate').text(rate);

    // Trends
    $('#mon-rec-today-trend').text(`${fmt(recMonth)} this month`);
    $('#mon-comp-today-trend').text(`${fmt(totalComp)} this year`);
    $('#mon-month-trend').text(`${fmt(totalRec)} total this year`);

    // ── Division charts ──
    renderMonitorDivisionBar('mon-division-received', data.apps_rec_divisional,      '#3b82f6', '#1e3a5f');
    renderMonitorDivisionBar('mon-division-completed', data.apps_comp_divisional,    '#10b981', '#1e3a5f');
    renderMonitorDivisionBar('mon-past-due',   data.apps_past_due_dates_divisional,  '#ef4444', '#1e3a5f');
    renderMonitorDivisionBar('mon-with-divisions', data.apps_at_division,            '#f59e0b', '#1e3a5f');
    ...
    $('#monitorLastUpdate').text(now);

    // ── Also update original dashboard ──
    refreshDashboardStats();
}

// ==================== DIVISION BAR RENDERER ====================
function renderMonitorDivisionBar(containerId, data, color, bg) {
    ...
    const html = data.map(item => {
        ...
        return `
            <div class="division-progress-item">
                <div class="division-label">
                    <span>${item.division}</span>
                    <span style="color: ${color};">${new Intl.NumberFormat().format(item.total)} 
                        <small>(${pct}%)</small>
                    </span>
                </div>
                <div class="progress">
                    <div class="progress-bar" 
                         role="progressbar"
                         style="width: 0%; background: ${color}; border-radius: 6px;"
    ...
    setTimeout(() => {
        container.find('.progress-bar').each(function() {
            const target = $(this).data('target-width');
            $(this).animate({ width: target + '%' }, 800);
        });
    }, 100);
}
...
function animateCounter(elementId, targetValue) {
    ...
    const timer = setInterval(() => {
        ...
        if (step >= steps) {
            clearInterval(timer);
            el.text(new Intl.NumberFormat().format(targetValue));
        }
    }, duration / steps);
}
...
function buildTicker(data) {
    ...
    // Add received today
    const recToday = data.apps_rec_day?.[0]?.total || 0;
    items.push(`<span class="monitor-ticker-item">
        <span class="ticker-badge" style="background:rgba(59,130,246,0.2);color:#60a5fa;">RECEIVED</span>
        ${new Intl.NumberFormat().format(recToday)} applications received today
    </span>`);

    // Add completed today
    const compToday = data.apps_comp_day?.[0]?.total || 0;
    items.push(`<span class="monitor-ticker-item">
        <span class="ticker-badge" style="background:rgba(16,185,129,0.2);color:#34d399;">COMPLETED</span>
        ${new Intl.NumberFormat().format(compToday)} applications completed today
    </span>`);

    // Add division breakdown
    if (data.apps_rec_divisional) {
        data.apps_rec_divisional.forEach(d => {
            items.push(`<span class="monitor-ticker-item">
                <span class="ticker-badge" style="background:rgba(139,92,246,0.2);color:#a78bfa;">${d.division}</span>
                ${new Intl.NumberFormat().format(d.total)} applications received
            </span>`);
        });
    }

    // Add past due count
    if (data.apps_past_due_dates_divisional) {
        const pastDueTotal = data.apps_past_due_dates_divisional
            .reduce((s, i) => s + (parseInt(i.total) || 0), 0);
        items.push(`<span class="monitor-ticker-item">
            <span class="ticker-badge" style="background:rgba(239,68,68,0.2);color:#f87171;">PAST DUE</span>
            ${new Intl.NumberFormat().format(pastDueTotal)} applications past due date
        </span>`);
    }
...
function showMonitorSkeletons() {
    ['mon-rec-today', 'mon-comp-today', 'mon-rec-month'].forEach(id => {
        $(`#${id}`).html('<div class="skeleton" style="height:50px;width:80px;"></div>');
    });
    $('#mon-completion-rate').html('<div class="skeleton" style="height:50px;width:80px;"></div>');

    ['mon-division-received', 'mon-division-completed', 'mon-past-due', 'mon-with-divisions']
    .forEach(id => {
        $(`#${id}`).html(`
...
function showMonitorError(msg) {
    $('#mon-division-received, #mon-division-completed, #mon-past-due, #mon-with-divisions')
        .html(`<p class="text-danger small text-center py-2"><i class="ri-wifi-off-line me-1"></i>${msg}</p>`);
}



  if ($("#page_name").text() === "compliance" || $("#page_name").text() === "compliance_no_login") {
    // Auto-select region with code 11
    const regionSelect = $('#sel_change_region_compliance');
    if (regionSelect.length && regionSelect.val() === "") {
        regionSelect.val("11").trigger('change');
    }
...
//     dateFromPicker.setDate(startDate, true);
//     dateToPicker.setDate(endDate, true);
...
//     $('#startdate').val(dateFromPicker.formatDate(startDateObj, "j F Y"));
//     $('#enddate').val(dateToPicker.formatDate(endDateObj, "j F Y"));
...
//     const initialEndDate   = "2023-06-30";

//     const startDateObj = new Date(initialStartDate);
//     const endDateObj   = new Date(initialEndDate);

//     dateFromPicker.setDate(initialStartDate, true);
//     dateToPicker.setDate(initialEndDate, true);
...
//     dateToPicker.set('minDate', initialStartDate);

//     startDate = initialStartDate;   // ← now correctly sets the outer let
//     endDate   = initialEndDate;
// });


// $(document).ready(function() {
//     const initialStart = "2023-01-01";
//     const initialEnd   = "2023-06-30";

//     const startDateObj = new Date(initialStart);
//     const endDateObj   = new Date(initialEnd);

//     // ✅ Set globals FIRST before flatpickr onChange fires
//     startDate = initialStart;
//     endDate   = initialEnd;

//     dateFromPicker.setDate(initialStart, true);
//     dateToPicker.setDate(initialEnd, true);

//     $('#start_date').val(initialStart);
//     $('#end_date').val(initialEnd);
//     $('#startdate').val(dateFromPicker.formatDate(startDateObj, "j F Y"));
//     $('#enddate').val(dateToPicker.formatDate(endDateObj, "j F Y"));

//     dateToPicker.set('minDate', initialStart);
// });
...
$(document).ready(function() {
    ...
    const initialStart = new Date(today.getFullYear(), 0, 1);
    ...
    const startStr = formatDate(initialStart);
    const endStr   = formatDate(initialEnd);
    ...
    dateFromPicker.setDate(startStr, true);
    dateToPicker.setDate(endStr, true);

    // Set hidden inputs
    $('#start_date').val(startStr);
    $('#end_date').val(endStr);

    // Set display inputs (formatted)
    $('#startdate').val(dateFromPicker.formatDate(initialStart, "j F Y"));
    $('#enddate').val(dateToPicker.formatDate(initialEnd, "j F Y"));

    // Prevent selecting end date before start date
    dateToPicker.set('minDate', startStr);
});
...
// Auto-reload if JS crashes in monitor mode
// window.onerror = function(msg, src, line, col, error) {
//     console.error('JS Error:', msg, src, line);
//     if (typeof isMonitorMode !== 'undefined' && isMonitorMode) {
...
window.onerror = function(msg, src, line, col, error) {
    ...
    // Do NOT reload — this exits monitor mode on any minor error
    return false;
};

// Auto-reload page every 4 hours to prevent memory leaks
setTimeout(() => {
    if (typeof isMonitorMode !== 'undefined' && isMonitorMode) window.location.reload();
}, 4 * 60 * 60 * 1000);
